...
public class TodoService {
	...
	public List<TodoEntity> create(TodoEntity entity){
		...
		//넘어온 Entity로부터 UserId를 가지고 DB에서 조회된 내용을 List에 묶어서 반환을 하겠다.
		//SElECT * FROM TodoEntity where userid = 'temporary-user';
		//실행해서 조회된 결과를 List에 묶어서 반환하겠다.라는 뜻
	...
	//Entity를 수정하고, 수정한 entity를 확인할 수 있는 updata메서드
	//TodoEntity entity 매개변수는 수정된 값이 들어있다.
	public List<TodoEntity> updata(TodoEntity entity){
		// 1. 수정할 entity가 유효한지 확인
		validate(entity);
		
		//존재하지 않는 entity는 업데이트 할 수 없기때문에
		//기존에 저장되어있는 데이터를 조회
...
//		방법 2)
		...
		return retrieve(entity.getUserId()); // 수정이 잘 되었는지 확인하기 위해 retrieve메서드를 실행한 결과를 반환.
